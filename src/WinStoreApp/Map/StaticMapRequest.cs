    ...
    public class StaticMapRequest
    {
        ...
        private string GetMarkersStr()
        {
            ...
            foreach (MapMarkers current in this.Markers)
            {
                ...
                var c = current.Color;
                if (current.Color.Equals(Color.FromArgb(0, 0, 0, 0)) == false)
                {
                    if (sb.Length > 0) sb.Append(Constants.PIPE_URL_ENCODED);

                    sb.AppendFormat("color:0x{0:X6}", current.Color.ToString().Remove(0, 3));
                }
                ...
                {
                    if (sb.Length > 0) sb.Append(Constants.PIPE_URL_ENCODED);
                    sb.AppendFormat("icon:{0}", WebUtility.UrlEncode(current.IconUrl));
